...
export class ClaudeCodeAgent implements Agent {
  ...
  async startTask(task: AgentTask): Promise<AgentRun> {
    ...
    // Materialize the artifacts the simulated session "created" into the
    // sandbox FS, so the grader inspects a real (simulated) filesystem rather
    // than nothing. The agent writes the GENERIC files a coding agent tends to
    // produce — it does NOT peek at the eval's assertions. That keeps the
    // outcome honest: an assertion expecting `src/checkout.ts` will fail
    // because the agent created `src/index.ts` instead — exactly the kind of
    // gap Kiln exists to surface.
    const head = task.prompt.split("\n", 1)[0]?.slice(0, 80) ?? "task";
    await task.sandbox.writeFile(
      "src/index.ts",
      `// Generated by the agent for: ${head}\n` +
        `import { Client } from "the-sdk";\n\n` +
        `const client = new Client(process.env.API_KEY);\n` +
        `export async function main() {\n  // integration logic\n  return client;\n}\n`,
    );
    await task.sandbox.writeFile(
      "test.js",
      `// Smoke test written by the agent.\nconsole.log("ok");\nprocess.exit(0);\n`,
    );
    await task.sandbox.writeFile(
      "README.md",
      `# Integration\n\nThis project integrates the SDK as described in the task:\n\n> ${head}\n\n` +
        `It initializes the client, performs the integration, and follows the SDK's documented patterns.\n`,
    );
